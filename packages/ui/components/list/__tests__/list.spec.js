...
describe("md-list", () => {
  ...
  describe("rendering", () => {
    it("renders the element", async () => {
      const el = /** @type {MdList} */ (
        await fixture(html`<md-list></md-list>`)
      );
      expect(el).to.exist;
    });
  ...
  describe("roving tabindex initialisation", () => {
    it("sets tabindex=0 on the first interactive item after slot population", async () => {
      ...
      const item1 = /** @type {MdListItem} */ (el.querySelector("#item1"));
      const item2 = /** @type {MdListItem} */ (el.querySelector("#item2"));
    ...
    it("ArrowDown navigation sets tabindex=-1 on the previously focused item", async () => {
      ...
      const btn1 = /** @type {HTMLButtonElement} */ (
        item1.shadowRoot.querySelector("button")
      );
      btn1.focus();
    ...
    it("does not assign tabindex to non-interactive items", async () => {
      ...
      const staticItem = /** @type {MdListItem} */ (
        el.querySelector("#static1")
      );
  ...
  describe("keyboard navigation — ArrowDown", () => {
    ...
    it("ArrowDown wraps from last item back to first", async () => {
      ...
      const btn2 = /** @type {HTMLButtonElement} */ (
        item2.shadowRoot.querySelector("button")
      );
      btn2.focus();
  ...
  describe("keyboard navigation — Home and End", () => {
    it("Home key focuses the first interactive item", async () => {
      ...
      const item1 = /** @type {MdListItem} */ (el.querySelector("#item1"));
      const item2 = /** @type {MdListItem} */ (el.querySelector("#item2"));
      const item3 = /** @type {MdListItem} */ (el.querySelector("#item3"));
      await item1.updateComplete;
      await item2.updateComplete;
      await item3.updateComplete;

      // Start from the last item
      const btn3 = /** @type {HTMLButtonElement} */ (
        item3.shadowRoot.querySelector("button")
      );
      btn3.focus();
  ...
  describe("non-interactive items are skipped during keyboard navigation", () => {
    it("ArrowDown skips static items between two interactive items", async () => {
      ...
      const btn1Item = /** @type {MdListItem} */ (el.querySelector("#btn1"));
      const btn2Item = /** @type {MdListItem} */ (el.querySelector("#btn2"));
      await btn1Item.updateComplete;
      await btn2Item.updateComplete;

      const btn1 = /** @type {HTMLButtonElement} */ (
        btn1Item.shadowRoot.querySelector("button")
      );
      btn1.focus();
    ...
    it("href items are treated as interactive (not skipped)", async () => {
      ...
      const link1 = /** @type {MdListItem} */ (el.querySelector("#link1"));
      const link2 = /** @type {MdListItem} */ (el.querySelector("#link2"));
      await link1.updateComplete;
      await link2.updateComplete;

      const anchor = /** @type {HTMLAnchorElement} */ (
        link1.shadowRoot.querySelector("a")
      );
      anchor.focus();
