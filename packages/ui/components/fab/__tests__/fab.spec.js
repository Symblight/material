...
describe("md-fab", () => {
  ...
  describe("rendering", () => {
    it("renders a <button> element by default", async () => {
      const el = /** @type {FAB} */ (await fixture(html`<md-fab></md-fab>`));
      expect(el.shadowRoot.querySelector("button")).to.exist;
    });

    it("renders an <a> element when href is set", async () => {
      const el = /** @type {FAB} */ (
        await fixture(html`<md-fab href="/home"></md-fab>`)
      );
  ...
  describe("variant", () => {
    ...
    for (const variant of ["surface", "primary", "secondary", "tertiary"]) {
      it(`sets variant="${variant}" attribute`, async () => {
        const el = /** @type {FAB} */ (
          await fixture(html`<md-fab variant=${variant}></md-fab>`)
        );
  ...
  describe("size", () => {
    ...
    for (const size of ["s", "m", "l"]) {
      it(`sets size="${size}" attribute`, async () => {
        const el = /** @type {FAB} */ (
          await fixture(html`<md-fab size=${size}></md-fab>`)
        );
  ...
  describe("label", () => {
    it("renders label text inside the button", async () => {
      const el = /** @type {FAB} */ (
        await fixture(html`<md-fab label="Create"></md-fab>`)
      );
  ...
  describe("disabled", () => {
    it("is not disabled by default", async () => {
      const el = /** @type {FAB} */ (await fixture(html`<md-fab></md-fab>`));
      expect(
        /** @type {HTMLButtonElement} */ (el.shadowRoot.querySelector("button"))
          .disabled,
      ).to.be.false;
    });
    ...
    it("disables the inner button when disabled", async () => {
      const el = /** @type {FAB} */ (
        await fixture(html`<md-fab disabled></md-fab>`)
      );
      expect(
        /** @type {HTMLButtonElement} */ (el.shadowRoot.querySelector("button"))
          .disabled,
