 ...
 * @returns {Element}
 */
function badgeInner(el) {
  return /** @type {Element} */ (
    /** @type {ShadowRoot} */ (el.shadowRoot).querySelector(".badge")
  );
}
...
describe("md-badge", () => {
  ...
  describe("rendering", () => {
    it("renders a .badge element", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge></md-badge>`)
      );
      expect(badgeInner(el)).to.exist;
    });
  ...
  describe("large (label) variant", () => {
    it("adds badge_large class when value is set", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="3"></md-badge>`)
      );
      const inner = badgeInner(el);
    ...
    it("displays the value as text", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="7"></md-badge>`)
      );
      const inner = badgeInner(el);
    ...
    it("displays a string value", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="new"></md-badge>`)
      );
      const inner = badgeInner(el);
  ...
  describe("max truncation", () => {
    it("shows '{max}+' when numeric value exceeds max", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="1200" .max=${999}></md-badge>`)
      );
      const inner = badgeInner(el);
    ...
    it("shows value as-is when equal to max", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="999" .max=${999}></md-badge>`)
      );
      const inner = badgeInner(el);
      expect(inner.textContent.trim()).to.equal("999");
    });
    ...
    it("shows value as-is when below max", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="42" .max=${999}></md-badge>`)
      );
      const inner = badgeInner(el);
    ...
    it("respects a custom max value", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="100" .max=${99}></md-badge>`)
      );
      const inner = badgeInner(el);
  ...
  describe("reactivity", () => {
    ...
    it("switches from large to small when value is cleared", async () => {
      const el = /** @type {MdBadge} */ (
        await fixture(html`<md-badge value="5"></md-badge>`)
      );
      el.value = "";
      await el.updateComplete;
      const inner = badgeInner(el);
