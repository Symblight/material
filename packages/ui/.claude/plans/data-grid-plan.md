...
components/data-grid/
  ...
  controllers/
    ...
    data-grid-sort-controller.js
    data-grid-row-span-controller.js
...
Also added since §15: `DataGridColumn.colSpan` (header cell spans N column tracks via `grid-column: span N`, now also applied to every row's data cell — the same `clampColSpan` helper + `coveredUntil` skip pattern is shared by the header loop and each row's cell loop in `data-grid.js`); `slot="empty-label"` overriding the default "No rows" text; `exportparts` on internal sub-component usages in `data-grid.js` so nested parts are reachable as `md-data-grid::part(...)` from outside; drag-to-resize (`ColumnResizeController` + `md-data-column-separator`); each `components/` sub-component given its own folder (`cell/`, `column-header/`, etc.), mirroring the top-level `components/<name>/` convention; click-to-sort (`SortController`, single-column `sortModel`, `md-data-column-title` gets a sort icon); MUI-style vertical row spanning (`RowSpanController`, opt-in via `rowSpanning` — off by default, unlike resize/sort — since silently merging cells is a bigger behavior change than a resize handle or sort click). Row spanning's owner cell renders taller by overflowing past its own row (`height: rowHeight * span` + `position: relative; z-index: 1` + an opaque background) rather than restructuring virtualization's per-row-independent-grid layout — verified via `elementFromPoint()` in a real browser that the owner cell, not the covered row beneath it, is what's actually on top. Documented tradeoff: if a run's owner row scrolls outside the virtualized window while covered rows are still visible, those rows show blank until the owner remounts.
